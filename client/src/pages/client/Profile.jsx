...
export default function Profile() {
  ...
  return <div className="mx-auto w-full max-w-2xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    <div className="overflow-hidden rounded-2xl border border-stone-200/80 bg-white shadow-sm"><form onSubmit={submit}>
      ...
      <section className="space-y-4 border-t border-stone-100 bg-stone-50/40 p-5 sm:p-6"><div className="flex items-start justify-between gap-4"><div><h3 className="text-sm font-bold text-stone-800">Sports</h3><p className="mt-1 text-xs text-stone-500">Your sports and current skill levels.</p></div>{!user?.sports?.length && !addingSport && <button type="button" onClick={() => { setStatus(null); setAddingSport(true); }} className="shrink-0 rounded-lg border border-stone-200 px-3 py-1.5 text-xs font-bold text-stone-600 hover:border-orange-200 hover:bg-orange-50 hover:text-orange-600">Add sport</button>}</div>
        {user?.sports?.length ? <div className="grid gap-3 sm:grid-cols-2">{user.sports.map((userSport) => <div key={userSport.id} className="rounded-xl border border-stone-200 bg-white px-3.5 py-3"><p className="text-sm font-bold capitalize text-stone-800">{userSport.sport}</p><p className="mt-0.5 text-xs text-stone-500">{skillLevels.find(([value]) => value === userSport.skillLevel)?.[1]}</p></div>)}</div> : addingSport && <div className="grid gap-4 sm:grid-cols-2"><div><label htmlFor="profile-sport" className={labelClass}>Sport</label><select id="profile-sport" value={sport} onChange={(event) => setSport(event.target.value)} className={`${inputClass} cursor-pointer`}><option value="badminton">Badminton</option><option value="volleyball">Volleyball</option></select></div><div><label htmlFor="sport-skill" className={labelClass}>Skill level</label><select id="sport-skill" value={sportSkillLevel} onChange={(event) => setSportSkillLevel(event.target.value)} className={`${inputClass} cursor-pointer`}>{skillLevels.map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></div><div className="sm:col-span-2 flex justify-end gap-2"><button type="button" onClick={() => setAddingSport(false)} className="rounded-xl px-4 py-2.5 text-xs font-bold text-stone-500 hover:bg-stone-100">Cancel</button><button type="button" onClick={addSport} disabled={saving} className={actionClass}>{saving && <Loader2 size={15} className="mr-2 inline animate-spin" />}Add sport</button></div></div>}
      </section>
